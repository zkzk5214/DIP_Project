...
%      2.  Apply exponential function in the filter
...
%  Date:        03/25/2020
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ad=Anisodiff(f,lam,k,iter)
[M,N]=size(f);
I=f;
IN=zeros(M,N);
a=zeros(M,N);
...
for p=0:iter
    for i =2:M-1
        for j = 2:N-1
            ...
            cn(i,j)=exp((-1)*power((abs(IN(i,j)/k)),2));
            cs(i,j)=exp((-1)*power((abs(IS(i,j)/k)),2));
            cw(i,j)=exp((-1)*power((abs(IW(i,j)/k)),2));
            ce(i,j)=exp((-1)*power((abs(IE(i,j)/k)),2));
