...
[M, N] = size(f);
% To get the histogramb pr(r) of image f
figure;
p1 = ph(f,M,N); 
bar(p1);
...
%% 3) Filtering the salt and pepper noise. Comment this part if image with
% size of the filter to remove noise;
% size set as 3*3
kernel=zeros(3,3);
for x=1:3
    for y=1:3
...
As=disk_lma(10);    % Generated disk A for smallest disk
Ab=disk_lma(36);    % Generated disk A for biggest disk
Bs=hole_lma(8);     % Generated w-a mask (B) for smallest disk
Bb=hole_lma(31);    % Generated w-a mask (B) for biggest disk
As = double(As);
Ab = double(Ab);
Bs = double(Bs);
Bb = double(Bb);
